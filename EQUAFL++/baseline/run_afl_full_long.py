...
if "mipsel" in text:
	QEMU = "./qemu-mipsel"
	arch = "mipsel"
	copy_str = "cp dependence/qemu-system-mipsel-full %s/qemu-system-mipsel"  %(image_dir)
	os.system(copy_str)
elif "mipseb" in text:
	QEMU = "./qemu-mips"
	arch = "mipseb"
	copy_str = "cp dependence/qemu-system-mips-full %s/qemu-system-mips" %(image_dir)
	os.system(copy_str)
elif "armel" in text:
	QEMU = "./qemu-arm"
	arch = "armel"
	copy_str = "cp dependence/qemu-system-arm-full %s/qemu-system-arm" %(image_dir)
	os.system(copy_str)
else:
	print("#########################error########################")
...
cmdstr = "cp dependence/afl-fuzz-full %s/" %image_dir
os.system(cmdstr)
cmdstr = "cp dependence/test.py %s/" %image_dir
os.system(cmdstr)
cmdstr = "python generate_run_full.py %s %s %d" %(image_id, arch, int(run_core_base)+ int(copy_id))
os.system(cmdstr)
...
os.system("rm -r outputs_full")

#(2) only run_full.sh
print("wait_time", wait_time, "\n\n\n\n")
timer = threading.Timer(wait_time, fun_timer, (ip,))
timer.start()

os.system("./run_full.sh")
